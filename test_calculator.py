...
def test_task_1():
    assert format_date("29/01/2026") == "2026-01-29"

def test_task_2():
    assert calculate_risk(20, 80, True) == 26.0 # (10 + 16) + 10

def test_task_3():
    # If there are two zeros, the original bug might only remove one!
    assert clean_results([1.2, 0.0, 0.0, 4.5]) == [1.2, 4.5]

def test_task_4():
    assert get_dosage("Asprin") == "100mg"
    assert get_dosage("Vitamin C") == "Not Found"

def test_task_5():
    assert average_heart_rate([]) == 0

def test_task_6():
    assert log_surgery(5, "Leen") == "Room 5: Leen"
